...
func (r *Reactor) applyAppendTx(peerID magmatypes.ServerID, tx []byte) (Result, error) {
	...
	var err error
	r.lastLogTerm, r.commitInfo.NextLogIndex, err = r.state.Append(tx, true)
	if err != nil {
...
func (r *Reactor) applyAppendEntriesACK(peerID magmatypes.ServerID, m *types.AppendEntriesACK) (Result, error) {
	...
	if m.NextLogIndex >= nextIndex && m.SyncLogIndex > r.matchIndex[peerID] {
		r.matchIndex[peerID] = m.SyncLogIndex
		r.updateLeaderCommit(m.SyncLogIndex)
	}

	if m.NextLogIndex > nextIndex {
		nextIndex = m.NextLogIndex
		// if err := pSync.Iterator.Acknowledge(uint64(m.NextLogIndex)); err != nil {
		// 	return r.resultError(err)
		// }
	}
...
func (r *Reactor) applyClientRequest(m *types.ClientRequest) (Result, error) {
	...
	var err error
	r.lastLogTerm, r.commitInfo.NextLogIndex, err = r.state.Append(m.Data, false)
	if err != nil {
		return r.resultError(err)
	}
